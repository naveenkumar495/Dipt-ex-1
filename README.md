...
#### 1. Read the image ('naveen.jpg') using OpenCV imread() as a grayscale image.
```python
import cv2
import numpy as np
import matplotlib.pyplot as plt
img =cv2.imread('naveen.jpg',cv2.IMREAD_COLOR)
...
#### 10. Read in the image ('naveen.jpg').
```python
img=cv2.imread('naveen.jpg',cv2.IMREAD_COLOR)
img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
img_rgb.shape
...
```python
text = cv2.putText(img_rgb, "naveen", (300, 700),cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)  
